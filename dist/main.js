...
async function getLocationCoordinates(location) {
  const coordinates = await fetch(`https://api.openweathermap.org/geo/1.0/direct?q=${location}&limit=1&appid=163a88340501cc3338e7b7a9919e470f`);
...
async function getYesterdayWeatherData(coordinates) {
  ...
  const formattedYesterday = (0,date_fns__WEBPACK_IMPORTED_MODULE_0__["default"])(yesterday, 'yyyy-MM-dd');
  console.log(yesterday);
  const apiData = await fetch(`https://api.openweathermap.org/data/3.0/onecall/day_summary?lat=${coordinates.lat}&lon=${coordinates.lon}&date=${formattedYesterday}&units=metric&appid=163a88340501cc3338e7b7a9919e470f`);
  const apiDataJson = await apiData.json();
  return apiDataJson;
}
async function getApiWeatherData(coordinates) {
  const apiData = await fetch(`https://api.openweathermap.org/data/3.0/onecall?lat=${coordinates.lat}&lon=${coordinates.lon}&units=metric&appid=163a88340501cc3338e7b7a9919e470f`);
  const apiDataJson = await apiData.json();
  return apiDataJson;
}
...
async function start() {
  ...
  const appView = new _appView__WEBPACK_IMPORTED_MODULE_2__["default"]({
    location: 'Kyiv'
  });
  document.body.appendChild(appView);
  const cordinates = await (0,_utils_getApiData__WEBPACK_IMPORTED_MODULE_3__.fetchApiData)('ferdjioua');
